...
def my_has_der (f f': ℝ → ℝ) (x  : ℝ) (s : Set ℝ ) : Prop :=
  ∀ ε > 0, ∃ δ > 0, ∀ y, |y-x| > 0 ∧ |y - x| < δ ∧ y ∈ s ∪ {x}  → |(f (y) - f x) / (y-x) - f' x| < ε
...
lemma my_has_der_unique (f f₁' f₂' : ℝ → ℝ) (x : ℝ) (s : Set ℝ)
  ...
  specialize hδ1 x
  simp at *
  have abs_inequality : |f₁' x - f₂' x| <= |f₁' x| + |f₂' x| := by
    have h : f₁' x - f₂' x = f₁' x + (-f₂' x) := by ring
    rw [h]
    have h' : |f₂' x| = |-f₂' x| := by simp
    rw [h']
    exact abs_add (f₁' x) (-f₂' x)
  have auxε : 2*ε = |f₁' x - f₂' x| := by
    unfold ε
    linarith
  rw [← auxε] at abs_inequality
  have smaller : |f₁' x| + |f₂' x| < 2*ε := by
...
theorem contdiffOn_equiv' (f : ℝ → ℝ) (n : WithTop ℕ∞) ( s : Set ℝ) :
  ...
  intro x hx
  specialize h x hx
  rw [contDiffWithinAt_infty] at h
  unfold my_contdiffwithin_at
  have hn' : n = ⊤ := by
    rw [hn]
    unfold m
    simp
...
lemma inverse_has_deriv_iterated {φ : ℝ → ℝ} {ψ: ℝ → ℝ} {a b c d : ℝ} (k : ℕ) (hφ : ∀ x, x ∈ Set.Ioo a b → φ x ∈ Set.Ioo c d)
    ...
    :
  ∃ (ψ' : ℝ → ℝ →L[ℝ] ℝ), (∀ y ∈ Ioo c d, HasFDerivWithinAt ψ (ψ' y) (Ioo c d) y) ∧ ContDiffOn ℝ k ψ' (Ioo c d) := by
  sorry
...
example (φ : ℝ → ℝ) (ψ : ℝ → ℝ) (a b c d : ℝ) (hab : a ≤ b) (hcd : c ≤ d)
    ...
    ContDiffOn ℝ ⊤ ψ (Set.Ioo c d) ∧
      ...
      have ψ_smooth: ContDiffOn ℝ ⊤ ψ (Ioo c d) := by
        let k : ℕ∞ := ⊤
        have aux : ↑k = (⊤ :WithTop ℕ∞) := by
          sorry
        rw [← aux]
        apply contDiffOn_infty.mpr
        intro m
        induction' m with m HI
        --Se procede de manera similar a la prueba de segunda parte
        sorry

        have hk : ContDiffOn ℝ m ψ (Ioo c d) := HI
        have aux': (↑(m + 1) : WithTop ℕ∞) = ↑(m) + 1 := by
            sorry
        rw [aux']
        rw [contDiffOn_succ_iff_hasFDerivWithinAt]
        intro y hy
        use Ioo c d
        constructor
        ...
        constructor
        simp
        obtain ⟨f', hf', hdiff⟩ := inverse_has_deriv_iterated m hφ hψ left_inv right_inv hφdiff hφregular
        exact ⟨f', hf', hdiff⟩
        simp
        ...
        have res : deriv φ (ψ y) = 1 / deriv ψ y := by
          have aux : HasDerivAt ψ (deriv φ (ψ y))⁻¹ y := by
            ...
            have ψ_cont : ContinuousAt ψ y := by
              have ψ_contdiffat : ContDiffAt ℝ ⊤ ψ y := by
                apply ContDiffOn.contDiffAt ψ_smooth
                rw [mem_nhds_iff]
                use Ioo c d
                constructor
                simp
                ...
                exact isOpen_Ioo
                exact hy
              apply ContDiffAt.continuousAt ψ_contdiffat

            obtain ⟨η, η_pos, hη⟩ : ∃ η > 0, ∀ y₁, |y₁ - y| < η → |ψ y₁ - ψ y| < δ' := by
              rw [cont] at ψ_cont
              unfold Mycont at ψ_cont
              specialize ψ_cont δ'
              apply ψ_cont at δ'_pos
              exact δ'_pos
            let r := min (y - c) (d - y)
            have r_pos : r > 0 := by
              unfold r
            ...
            have y₁_close' : |y₁ - y| < η := by
              unfold δ at y₁_close
              rw [lt_min_iff] at y₁_close
              exact y₁_close.right
            have y₁_close'''' : |y₁ - y| < r := by
              unfold δ at y₁_close
              rw [lt_min_iff] at y₁_close
              exact y₁_close.left
            have hψ_close : |ψ y₁ - ψ y| < δ' := hη y₁ y₁_close'
            have in_Ioo : y₁ ∈ Ioo c d := by
              rcases hy with ⟨hcy, hyd⟩
              have h_r_min : r ≤ y - c ∧ r ≤ d - y := by
                unfold r
                exact ⟨inf_le_left, inf_le_right⟩
              rw [abs_lt] at y₁_close''''
              rcases y₁_close'''' with ⟨h_yl, h_yr⟩
              have h_yl_new : y - r < y₁ := by linarith [h_yl]
              have h_yr_new : y₁ < y + r := by linarith [h_yr]
              have hc_y₁ : c < y₁ := by
                calc
                  c = y - (y - c) := by ring
                  _ ≤ y - r := by gcongr; exact h_r_min.1
                  _ < y₁ := h_yl_new
              have hy₁_d : y₁ < d := by
                calc
                  y₁ < y + r := h_yr_new
                  _ ≤ y + (d - y) := by gcongr; exact h_r_min.2
                  _ = d := by ring
              exact ⟨hc_y₁, hy₁_d⟩
            ...
            have dφ_r_ne_zero : dφ + r ≠ 0 := by
              have aux1: |r| < |dφ|/2 := lt_of_lt_of_le r_bound (min_le_left _ _)
              have aux2: |dφ + r| ≥ |dφ| - |r| := by
                have h := abs_sub_abs_le_abs_sub dφ  (-r)
                simp at h
                linarith [h]
              have : |dφ| - |r| > |dφ| / 2 := by linarith [aux1]
              have : |dφ + r| > |dφ| / 2 := by
                linarith
              have h_abs_nonneg : |dφ| / 2 ≥ 0 := by linarith [abs_nonneg dφ]
              have h_pos : |dφ + r| > 0 := by linarith [this, h_abs_nonneg]
              rw [← abs_pos]
              exact h_pos
            ...
            have : (ψ y₁ - ψ y) / (y₁ - y) = 1 / (dφ + r) := by
              ...
              ring
              field_simp [sub_ne_zero.mpr hψ_eq]
              ring_nf
