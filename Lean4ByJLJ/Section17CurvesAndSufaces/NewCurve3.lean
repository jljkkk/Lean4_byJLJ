...
def IccTopology : TopologicalSpace (Icc a b) :=
  TopologicalSpace.induced (fun (x : Icc a b) => (x : ℝ)) inferInstance

instance : TopologicalSpace (Icc a b) :=
  TopologicalSpace.induced (fun x => (x : ℝ)) inferInstance

theorem Ioc_isOpen_in_Icc : IsOpen (Ioc a b ) := by

  have h1 : Ioc a b = Ioo a (b+1) ∩ Icc a b := by
    ext x
    constructor
    intro h
    constructor
...
theorem Ico_isOpen_in_Icc : IsOpen (Ico a b) := by
  rw [isOpen_induced_iff]
  use Ioo a b
  constructor
  · exact isOpen_Ioo
  · ext x
    simp [Ioc, Ioo, le_refl]

theorem univ_isOpen: IsOpen (Icc a b) := by
  apply isOpen_univ
...
example (φ : ℝ → ℝ) (ψ : ℝ → ℝ) (a b c d : ℝ) (hab : a ≤ b) (hcd : c ≤ d)
    ...
    ContDiffOn ℝ ⊤ ψ (Set.Icc c d) ∧
      ...
      have ψ_smooth : ContDiffOn ℝ ⊤ ψ (Icc c d) := by
        intro y hy
        have hx : ψ y ∈ Icc a b := hψ y hy
        have hφ_smooth_at : ContDiffAt ℝ ⊤ φ (ψ y) := by
          cases' hx with ha hb
          cases' lt_or_eq_of_le ha with ha_lt ha_eq
          cases' lt_or_eq_of_le hb with hb_lt hb_eq
          /-Case (a,b)-/
          have h: Icc a b ∈ nhds (ψ y) := by
            rw [mem_nhds_iff]
            use Ioo a b
            constructor
        ...
        have local_inv: ψ =ᶠ[nhds (φ x)] (hφ_smooth_at.localInverse hf' (by simp)) := by
          ...
          /-
          Maybe its possible to apply contradicction using ContinuousAt.congr
          -/
          ...
          have left_inv_eventually : ∀ᶠ z in nhds x, ψ (φ z) = z := by
            ...
            constructor
            apply univ_isOpen
            exact hx /-exact hx_mem, need some fix here-/
        ...
        have : ContDiffWithinAt ℝ ⊤ ψ (Icc c d) y := by
          exact this.contDiffWithinAt

        exact this


      have hψ_diff : ∀ y ∈ Icc c d, DifferentiableAt ℝ ψ y := by
        sorry

      have hφ_diff : ∀ y ∈ Icc c d, DifferentiableAt ℝ φ (ψ y) := by
        intro y hy
        apply differentiableAt_of_deriv_ne_zero
      ...
      have h_id : ∀ y ∈ Icc c d, φ (ψ y) = id y := by
        intro y hy
        apply right_inv
        exact hy

      have h_id_deriv : ∀ y ∈ Icc c d , deriv (φ ∘ ψ) y = deriv id y := by
        intro y hy
      ...
      have chain : ∀ y ∈ Icc c d, deriv  (φ ∘ ψ) y = (deriv φ (ψ y)) * (deriv ψ y):= by
        intro y hy
      ...
      have h_id_deriv1 : ∀ y ∈ Icc c d, (deriv φ (ψ y)) * (deriv ψ y) = 1 := by
        intro y hy
        rw [← chain y hy, h_id_deriv y hy]
        simp
